...
router.post('/createuser', [
    ...
    try {
        ...
        if (user) {
            return res.status(400).json({success, error: "User with the same email is already exist" });
        }
...
router.post('/login', [
    ...
    try {
        ...
        if (!user) {
            return res.status(400).json({success, error: "Please enter valid details to login" });
        }
